...
export class FilmComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.filmService
      .GetFilmInfo(Number(this.segmentValue))
      .subscribe((res: Film) => {
        this.film = res;
        this.filmService
          .GetSchedule(Number(this.segmentValue))
  ...
  addCinema() {
    ...
    for (let i = 0; i < this.schedule.length; i++) {
      if (
        this.cinemas.find(
          (c) => c.cinemaName === this.schedule[i].cinemaName,
        ) === undefined
      ) {
        this.cinemas.push({
  ...
  getDayMonth() {
    ...
    for (let i = 0; i < this.dates.length; i++) {
      if (
        this.dayMonths.find(
          (dm) => this.removeTime(dm.date) === this.removeTime(this.dates[i]),
        ) === undefined
      ) {
        this.dayMonths.push({
          date: this.dates[i],
          dateValue:
            ...
            ' ' +
            this.parseMonth(this.dates[i].getMonth()),
        });
      }
    }
  }
    ...
    if (counter % 2 !== 0) {
      ...
      for (let i = 0; i < this.schedule.length; i++) {
        ...
        if (cinema.cinemaId === this.schedule[i].cinemaId) {
          ...
          if (
            this.cinemaHalls.find(
              (ch) => ch.cinemaHallName === this.schedule[i].cinemaHallName,
            ) === undefined
          ) {
      ...
      for (let i = 0; i < this.cinemaHalls.length; i++) {
        (
          document.getElementById(
            'hall-' + String(this.cinemaHalls[i].cinemaHallId),
          ) as HTMLElement
        ).style.backgroundColor = 'transparent';
      }
      (
        document.getElementById(
          'hall-' + String(hall.cinemaHallId),
        ) as HTMLElement
    ...
    if (counter % 2 !== 0) {
      ...
      for (let i = 0; i < this.schedule.length; i++) {
        if (
          ...
          if (
            this.selectedHall !== 0 &&
            this.selectedHall === this.schedule[i].cinemaHallId
          ) {
            if (
              this.times.find(
                (t) =>
          ...
          if (
            this.selectedHall === 0 &&
            this.selectedCinema === this.schedule[i].cinemaId
          ) {
            if (
              this.times.find(
                (t) =>
      ...
      for (let i = 0; i < this.dayMonths.length; i++) {
        (
          document.getElementById(
            'date-' + String(this.dayMonths[i].date),
          ) as HTMLElement
        ).style.backgroundColor = 'transparent';
      }
      (
        document.getElementById('date-' + String(date.date)) as HTMLElement
      ).style.backgroundColor = '#1DE782';
    } else {
      //Если ничего не выбрано
      (
  ...
  onTimeSelected(time: Time) {
    ...
    if (counter % 2 !== 0) {
      ...
      for (let i = 0; i < this.times.length; i++) {
        (
          document.getElementById(
            String(this.times[i].time) + String(this.times[i].cinemaHallId),
          ) as HTMLElement
        ).style.backgroundColor = 'transparent';
      }
      (
        document.getElementById(
          String(time.time) + String(time.cinemaHallId),
        ) as HTMLElement
      ).style.backgroundColor = '#1DE782';
  ...
  buildConfirm(time: Time) {
    for (let i = 0; i < this.schedule.length; i++) {
      if (
        this.schedule[i].cinemaHallId === time.cinemaHallId &&
        new Date(Date.parse(this.schedule[i].dataTimeSession)).getTime() ===
          time.time.getTime()
      ) {
